package suscripciones

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"
)

// New creates a new suscripciones API client.
func New(transport runtime.ClientTransport, formats strfmt.Registry) *Client {
	return &Client{transport: transport, formats: formats}
}

/*
Client for suscripciones API
*/
type Client struct {
	transport runtime.ClientTransport
	formats   strfmt.Registry
}

/*
POSTSuscripciones crears suscripción
*/
func (a *Client) POSTSuscripciones(params *POSTSuscripcionesParams, authInfo runtime.ClientAuthInfoWriter) (*POSTSuscripcionesOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewPOSTSuscripcionesParams()
	}

	result, err := a.transport.Submit(&runtime.ClientOperation{
		ID:                 "POST_suscripciones",
		Method:             "POST",
		PathPattern:        "/suscripciones",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &POSTSuscripcionesReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	})
	if err != nil {
		return nil, err
	}
	return result.(*POSTSuscripcionesOK), nil

}

// SetTransport changes the transport on the client
func (a *Client) SetTransport(transport runtime.ClientTransport) {
	a.transport = transport
}
